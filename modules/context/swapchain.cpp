...
void transitionImageLayout(
  VkDevice device, VkCommandPool commandPool, VkQueue graphicsQueue, VkImage image, VkFormat format,
...
void ContextBare::makeSwapchain() {
    ...
    auto createImage = [this](
        ...
        allocInfo.memoryTypeIndex = memoryRequirements( memRequirements.memoryTypeBits, properties );
    ...
    createImage(
      viewportWidth, viewportHeight, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage_, depthImageMemory_ );

    auto createImageView = [this]( VkImage image, VkFormat format, VkImageAspectFlags aspectFlags ) -> VkImageView {
    ...
    transitionImageLayout(
      device_, commandPool_, graphicsQueue_, depthImage_, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED,
...
void ContextBare::makeFrames() {
    ...
    for ( size_t i{}; i < framesCount_; ++i ) {
        ...
        // Frame framebuffer
        std::array<VkImageView, 2> attachments = { frames_[i].view_, depthImageView_ };
        const auto [viewportWidth, viewportHeight] = viewportSize();
