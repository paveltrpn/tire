...
void RenderGL::configureGl() {
    ...
    glContext_ = glXCreateContextAttribsARB( display_, bestFbc_, nullptr, True, context_attribs.data() );
...
void RenderGL::displayRenderInfo() {
    spdlog::info( "OpenGL context info\nvendor - {}\nrenderer - {}\nOpenGL version - {}\nGLSL version - {}",
                  getGLVendorString(), getGLRendererString(), getGLVersionString(), getGLSLVendorString() );
}

void RenderGL::prepareShaders() {
    auto program = opengl::Shader{};
    program.link( { { GL_VERTEX_SHADER, shaderSourcesManager_.getVertexShader( "basic_color" ) },
