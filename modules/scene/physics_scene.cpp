...
PhysicsScene::PhysicsScene( const std::filesystem::path &fname ) {
    ...
    if ( file ) {
        ...
        throw std::runtime_error(
            std::format( "PhysicsScene ===  file not found: {}\n",
...
void PhysicsScene::initScene() {
    ...
    if ( scene_.contains( "objects" ) ) {
        ...
        for ( const auto &item : objects ) {
            ...
            if ( type == constants::scene::PARAM_NAME_OBJECT_TYPE_BOX ) {
                ...
                log::debug<DEBUG_OUTPUT_SCENE_CPP>(
                    "PhysicsScene === box added to scene" );
            } else if ( type ==
                        constants::scene::PARAM_NAME_OBJECT_TYPE_FRAME ) {
                ...
                log::debug<DEBUG_OUTPUT_SCENE_CPP>(
                    "PhysicsScene === frame added to scene" );
            } else if ( type ==
                        constants::scene::PARAM_NAME_OBJECT_TYPE_DIMOND ) {
                ...
                log::debug<DEBUG_OUTPUT_SCENE_CPP>(
                    "PhysicsScene === diamond added to scene" );
            }
        }
    } else {
    ...
    if ( scene_.contains( "cameras" ) ) {
        ...
        for ( const auto &item : cameras ) {
            ...
            if ( type == "perspective" ) {
                ...
                log::debug<DEBUG_OUTPUT_SCENE_CPP>(
                    "PhysicsScene === perspective camera added to scene" );
            }
        }
    } else {
...
void PhysicsScene::traverse( float duration ) {
