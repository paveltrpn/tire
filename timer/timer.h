...
// Наносекунда 10^-9
uint64_t timer_c::timespec_to_nanosec(const struct timespec &time) {
    return time.tv_sec*1000000000 + time.tv_nsec;
}

// Микросекунда 10^-6
uint64_t timer_c::timespec_to_microsec(const struct timespec &time) {
    return time.tv_sec*1000000 + time.tv_nsec/1000;
}

// Миллисекунда 10^-3
uint64_t timer_c::timespec_to_millisec(const struct timespec &time) {
    return time.tv_sec*1000 + time.tv_nsec/1000000;
}

uint64_t timer_c::timespec_to_seconds(const struct timespec &time) {
    return time.tv_sec;
}

uint64_t timer_c::timespec_to_minutes(const struct timespec &time) {
    return time.tv_sec/60;
}

uint64_t timer_c::timespec_to_hours(const struct timespec &time) {
...
uint64_t timer_c::get_delta_millisec() {
    uint64_t rt;
