...
class qtnn {
    public:
        ...
        qtnn() : data{ 0.0, 0.0, 0.0, 0.0 } {};

        qtnn(const float x, const float y, const float z, const float w) : data{ x, y, z, w } {};

        qtnn(const vec3 &v) : data{ v[0], v[1], v[2], 0.0 } {};

        qtnn(const qtnn &q) : data{ q[0], q[1], q[2], q[3] } {};
