...
bool CBitmap::readFromTgaFile(const std::string& fname) {
    ...
    if (!TGA_SUCCEEDED(in)) {
		std::cout << "CBitmap::readFromTgaFile(): ERROR! Unable to open file - " << fname << std::endl;
		return false;
	}
    ...
    std::memcpy(decompressed, static_cast<uint8_t*>(in_data.img_data), bitmapWidth*bitmapHeight*(bitmapChannels));
...
bool CBitmap::readFromJpegFile(const std::string& fname) { 
	...
	if (!fin) {
		std::cout << "CBitmap::readFromJpegFile(): ERROR! Unable to open file - " << fname << std::endl;
		return false;
	}
...
bool CBitmap::readFromBmpFile(const std::string& fname) {
    ...
    if (!fileStream) {
        std::cout << "CBitmap::readFromBmpFile(): Error opening file '" << fname << "." << std::endl;
        return 0;
    }
    ...
    if (file_header.bfType != 0x4D42) {
        std::cout << "CBitmap::readFromBmpFile(): Error -'" << fname << "' is not BMP file." << std::endl;
        return 0;
    }
    ...
    if (info_header.biCompression != 0 && info_header.biCompression != 3) {
        std::cout << "CBitmap::readFromBmpFile(): Error - Unsupported BMP compression." << std::endl;
        return 0;
    }
