...
std::vector<glm::vec2> boxTexCoords = {
    ...
    if (bdy != bodyList.end()) {
        ...
        for (size_t i = 0; i < bdy->second.bodyTriangles.size() / 3; i++) {
            bdy->second.bodyTriangles[i * 3 + 0]
              = glm::vec3(mAffine * glm::vec4(bdy->second.bodyTriangles[i * 3 + 0], 1.0f));
            bdy->second.bodyTriangles[i * 3 + 1]
              = glm::vec3(mAffine * glm::vec4(bdy->second.bodyTriangles[i * 3 + 1], 1.0f));
            bdy->second.bodyTriangles[i * 3 + 2]
              = glm::vec3(mAffine * glm::vec4(bdy->second.bodyTriangles[i * 3 + 2], 1.0f));

            bdy->second.bodyNormals[i * 3 + 0]
              = glm::vec3(mRotate * glm::vec4(bdy->second.bodyNormals[i * 3 + 0], 1.0f));
            bdy->second.bodyNormals[i * 3 + 1]
              = glm::vec3(mRotate * glm::vec4(bdy->second.bodyNormals[i * 3 + 1], 1.0f));
            bdy->second.bodyNormals[i * 3 + 2]
              = glm::vec3(mRotate * glm::vec4(bdy->second.bodyNormals[i * 3 + 2], 1.0f));
        }
