...
int main(int argc, char **argv) {
    ...
    for (size_t angl = 0; angl <= 180; angl = angl + 15) {
        ...
        anvas.brasenham_line({ int32_t(sin_i * CIRCLE_SIZE) + CANVAS_ZERO + 93,
                               int32_t(cos_i * CIRCLE_SIZE) + CANVAS_ZERO + 93 },
                             { -int32_t(sin_i * CIRCLE_SIZE) + CANVAS_ZERO + 93,
                               -int32_t(cos_i * CIRCLE_SIZE) + CANVAS_ZERO + 93 });
    }
    ...
    for (size_t angl = 0; angl <= 180; angl = angl + 15) {
        ...
        anvas.wu_line({ int32_t(sin_i * CIRCLE_SIZE) + CANVAS_ZERO - 101,
                        int32_t(cos_i * CIRCLE_SIZE) + CANVAS_ZERO - 101 },
                      { -int32_t(sin_i * CIRCLE_SIZE) + CANVAS_ZERO - 101,
                        -int32_t(cos_i * CIRCLE_SIZE) + CANVAS_ZERO - 101 });
    }
    ...
    for (size_t angl = 0; angl <= 180; angl = angl + 15) {
        ...
        anvas.dda_line({ int32_t(sin_i * CIRCLE_SIZE) + CANVAS_ZERO + 45,
                         int32_t(cos_i * CIRCLE_SIZE) + CANVAS_ZERO - 114 },
                       { -int32_t(sin_i * CIRCLE_SIZE) + CANVAS_ZERO + 45,
                         -int32_t(cos_i * CIRCLE_SIZE) + CANVAS_ZERO - 114 });
    }
