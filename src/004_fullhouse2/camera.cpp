...
void CPerspLookAtCamera::moveViewPointsSideway(float spd) {
    cmrTarget
      = vec3Sum(cmrTarget, { cmrLeftVec[0] * spd, cmrLeftVec[1] * spd, cmrLeftVec[2] * spd });
...
void CPerspLookAtCamera::moveViewPointsForward(float spd) {
    cmrTarget = vec3Sum(cmrTarget,
                        { cmrForwardVec[0] * spd, cmrForwardVec[1] * spd, cmrForwardVec[2] * spd });
    cmrEye
      = vec3Sum(cmrEye, { cmrForwardVec[0] * spd, cmrForwardVec[1] * spd, cmrForwardVec[2] * spd });
};
