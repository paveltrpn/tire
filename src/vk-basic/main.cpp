...
void doVulkanStuff() {
    ...
    std::for_each(extProperties.begin(), extProperties.end(), [&extNames](auto& ep) {
    ...
    std::for_each(layerProperties.begin(), layerProperties.end(), [&layerNames](auto& lp) {
    ...
    if (vkCreateDebugUtilsMessengerEXT(appInstance, &dbgCreateInfo, nullptr, &debugMessenger)
        != VK_SUCCESS) {
