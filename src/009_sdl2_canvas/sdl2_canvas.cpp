...
void mainApp_c::init_app() {
    if (SDL_Init(SDL_INIT_VIDEO) < 0) {
        std::cout << "mainApp_c::init_app(): Error - Unable to init SDL; " << SDL_GetError()
                  << std::endl;
        exit(1);
    }
    ...
    for (size_t angl = 0; angl <= 180; angl = angl + 15) {
        ...
        background.brasenham_line({ int32_t(sin_i * CIRCLE_SIZE) + CANVAS_ZERO,
                                    int32_t(cos_i * CIRCLE_SIZE) + CANVAS_ZERO },
                                  { -int32_t(sin_i * CIRCLE_SIZE) + CANVAS_ZERO,
                                    -int32_t(cos_i * CIRCLE_SIZE) + CANVAS_ZERO });
    }
    ...
    window = SDL_CreateWindow(
      "Cube", wnd_posx, wnd_posy, wnd_width, wnd_height, SDL_WINDOW_SHOWN | SDL_WINDOW_OPENGL);
    ...
    */
    my_canvas = SDL_CreateRGBSurfaceFrom(background.get_data(),
                                         wnd_width,
                                         wnd_height,
