...
void mainApp_c::get_gl_properties() {
    ...
    glsl_version
      = glsl_version + "GLSL_VERSION - " + (const char*)glGetString(GL_SHADING_LANGUAGE_VERSION);
}
...
void mainApp_sdl_c::init_app(int argc, char* argv[]) {
    ...
    if (SDL_Init(SDL_INIT_VIDEO) < 0) {
        std::cout << "mainApp_sdl_c::init_app(): Error - Unable to init SDL; " << SDL_GetError()
                  << std::endl;
        exit(1);
    }
    ...
    window
      = SDL_CreateWindow(appName, x_pos, x_pos, x_res, y_res, SDL_WINDOW_SHOWN | SDL_WINDOW_OPENGL);
...
void mainApp_X11_c::init_app(int argc, char* argv[]) {
    ...
    if (screenId != visual->screen) {
        std::cout << "mainApp_X11_c::init_app(): Error - screenId(" << screenId
