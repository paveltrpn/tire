...
// };
//
// CLOCK_REALTIME (системные часы) - значение этих часов содержит время в секундах и наносекундах,
// истекшее с 0 часов 1 января 1970 года.
//
// CLOCK_REALTIME_COARSE (начиная с Linux 2.6.32; есть только в Linux) - более быстрая, но менее
// точная версия CLOCK_REALTIME. Используйте, если нужны не очень точные метки времени, но быстро.
//
// CLOCK_MONOTONIC - часы, которые не могут быть настроены и показывают монотонный ход времени
// отсчитываемой с некой неопределённой начальной точки. Эти часы не подвержены скачкам системного
// времени (например, системный администратор вручную изменил время), но на них влияет постепенная
// подгонка, выполняемая adjtime(3) и NTP.
//
// CLOCK_MONOTONIC_COARSE (начиная с Linux 2.6.32; есть только в Linux) - более быстрая, но менее
// точная версия CLOCK_MONOTONIC. Используйте, если нужны не очень точные метки времени, но быстро.
//
// CLOCK_MONOTONIC_RAW (начиная с Linux 2.6.28; есть только в Linux) - похожи на CLOCK_MONOTONIC, но
// предоставляют прямой доступ к аппаратным часам, которые не подводятся NTP или постепенной
// подгонкой, выполняемой adjtime(3).
//
// CLOCK_BOOTTIME (начиная с Linux 2.6.39; есть только в Linux) - идентичны CLOCK_MONOTONIC, но
// также содержат любое время, на которое система была приостановлена (suspended). Это позволяет
// приложениям получить учитывающие приостановку монотонные часы без обращения к сложностям
// CLOCK_REALTIME, которые могут быть неоднородны, если время изменили с помощью settimeofday(2).
//
// CLOCK_PROCESS_CPUTIME_ID (начиная с Linux 2.6.12) - настраиваемые для каждого процесса часы ЦП
...
class timer_c {
    ...
    public:
        ...
        void show_timer_stats() {
            ...
            std::cout << "timer_c::clk_res_REALTIME = " << timespec_to_nanosec(clk_res_REALTIME)
                      << " ns; " << timespec_to_microsec(clk_res_REALTIME) << " mks; "
            ...
            std::cout << "timer_c::clk_res_MONOTONIC = " << timespec_to_nanosec(clk_res_MONOTONIC)
                      << " ns; " << timespec_to_microsec(clk_res_MONOTONIC) << " mks; "
            ...
            std::cout << "timer_c::clk_start_REALTIME = " << timespec_to_nanosec(clk_start_REALTIME)
                      << " ns; " << timespec_to_microsec(clk_start_REALTIME) << " mks; "
            ...
            std::cout << std::get<0>(clk_start_REALTIME_hr) << ":"
                      << std::get<1>(clk_start_REALTIME_hr) << ":"
                      << std::get<2>(clk_start_REALTIME_hr) << "\n";
            ...
            std::cout << std::get<0>(clk_start_MONOTONIC_hr) << ":"
                      << std::get<1>(clk_start_MONOTONIC_hr) << ":"
                      << std::get<2>(clk_start_MONOTONIC_hr) << "\n";
        };
