...
void initGlfwWindow(appState_s& appState) {
    ...
    appState.glfwWndPtr = glfwCreateWindow(
      appState.wndWidth, appState.wndHeight, appState.appName.c_str(), nullptr, nullptr);
    ...
    appState.glslVersionStr
      = std::string(reinterpret_cast<char const*>(glGetString(GL_SHADING_LANGUAGE_VERSION)));
}

void registerGlfwCallbacks(appState_s& appState) {
    // Нужно чтобы отправить appState в static storage duration, иначе лямбда не захватыват appState
    // из аргументов функции, а любое изменение параметров захвата лямбды приводит к тому, что её
    // тип перестаёт совпадать с GLFWkeyfun
