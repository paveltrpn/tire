...
void initWater() {
    ...
    g_fieldPoints
      = std::vector<glm::vec3>(c_fieldPointCount * c_fieldPointCount, glm::vec3(0.0f, 0.0f, 0.0f));
    ...
    for (auto i = 0; i < c_fieldPointCount; i++) {
        for (auto j = 0; j < c_fieldPointCount; j++) {
            g_fieldPoints[idRw(i, j, c_fieldPointCount)][0]
              = c_fieldSize - (2.0f * c_fieldSize) * i / static_cast<float>(c_fieldPointCount - 1);
            g_fieldPoints[idRw(i, j, c_fieldPointCount)][1]
              = c_fieldSize - (2.0f * c_fieldSize) * j / static_cast<float>(c_fieldPointCount - 1);
            g_fieldNormals[idRw(i, j, c_fieldPointCount)][2]
              = -4.0f / static_cast<float>(c_fieldPointCount - 1);
        }
    }
};
...
void waterDoStep() {
    ...
    for (i = 1; i < c_fieldPointCount - 1; i++) {
        for (j = 1; j < c_fieldPointCount - 1; j++) {
            g_fieldNormals[idRw(i, j, c_fieldPointCount)][0]
              = g_fieldNext[idRw(i - 1, j, c_fieldPointCount)]
                - g_fieldNext[idRw(i + 1, j, c_fieldPointCount)];
            g_fieldNormals[idRw(i, j, c_fieldPointCount)][1]
              = g_fieldNext[idRw(i, j - 1, c_fieldPointCount)]
                - g_fieldNext[idRw(i, j + 1, c_fieldPointCount)];
            g_fieldPoints[idRw(i, j, c_fieldPointCount)][2]
            ...
            g_fieldPrev[idRw(i, j, c_fieldPointCount)]
              = ((2.0f - c_viscosity) * g_fieldNext[idRw(i, j, c_fieldPointCount)]
...
void appSetup() {
    ...
    std::vector<glm::vec4> lightPosition
      = { { 1.0f, 1.0f, -2.0f, 1.0f }, { -35.0f, -5.0f, -20.0f, 1.0f } };
    ...
    glTexImage2D(GL_TEXTURE_2D,
                 0,
                 tmpBitmap.getChanelsCount(),
                 tmpBitmap.getWidht(),
                 tmpBitmap.getHeight(),
                 0,
                 GL_RGB,
                 GL_UNSIGNED_BYTE,
                 tmpBitmap.getDataPtr());
...
void appLoop() {
    ...
    while (!glfwWindowShouldClose(g_appWindow)) {
        ...
        ImGui::NewFrame();
        {
            ...
            ImGui::Text("Water modeling demo. Frame time - %.3f ms/frame (%.1f FPS)",
